...
class MiniMaxAlgorithm:
    ...
    def search(self, state, depth, maximizing_player):
        ...
        successors = state.get_possible_moves()

        if (not successors or depth == 0):
            return self.utility(state), None

        action, current = (max, (-INFINITY, None)) if maximizing_player \
            else (min, (INFINITY, None))
        ...
        for c in successors:
            ...
            s.perform_move(*c)
            t = (self.search(s, depth - 1, not maximizing_player)[0], c)
            current = action(t, current, key = lambda x: x[0])
...
class MiniMaxWithAlphaBetaPruning:
    ...
    def search(self, state, depth, alpha, beta, maximizing_player):
        ...
        for c in successors:
            ...
            s = copy.deepcopy(state)
            s.perform_move(*c)
            t = (self.search(s, depth - 1, alpha, beta, not maximizing_player)[0], c)
            current = action(t, current, key = lambda x: x[0])

            if maximizing_player:
                alpha = max(alpha, current[0])
                if current[0] >= beta:
                    return INFINITY, None
            else:
                beta = min(beta, current[0])
                if current[0] <= alpha:
                    return  -INFINITY, None
